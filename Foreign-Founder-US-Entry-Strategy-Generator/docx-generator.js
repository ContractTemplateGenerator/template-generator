...
<head>
<meta charset="UTF-8">
<title>${formData.documentTitle || "Foreign Founder U.S. Entry Strategy Agreement"}</title>
<style>
  body {
...
`;
    ...
    const documentHtml = documentText
      .split('\n\n')
      .map(para => para.trim() ? `<p>${para.replace(/\n/g, '<br>')}</p>` : '')
      .join('');
    
    // Add document text to HTML content
    ...
    link.download = `${formData.fileName || 'Foreign-Founder-US-Entry-Strategy-Agreement'}.doc`;
