              ...
              <div className="card">
                ...
                <div className="button-row" style={{display: "flex", gap: "1rem"}}>
                  ...
                  <button
                    className="nav-button next-button"
                    onClick={() => {
                      // Create Word document and download
                      let docContent = ndaText;
                      
                      // If using pseudonyms, add page break before side letter
                      if (formData.usePseudonyms) {
                        // Find where the side letter begins
                        const sideletterPos = docContent.indexOf("EXHIBIT A - IDENTITY CONFIRMATION LETTER");
                        if (sideletterPos !== -1) {
                          // Insert page break before side letter
                          docContent = docContent.substring(0, sideletterPos) + 
                                      "\n\n<br clear=all style='page-break-before:always'>\n\n" + 
                                      docContent.substring(sideletterPos);
                        }
                      }
                      
                      // Convert to MS Word compatible HTML
                      const wordDoc = `
                      ...
                      link.download = 'Non-Disclosure-Agreement.doc';
